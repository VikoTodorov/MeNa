...
if(isset($_POST['register-submit'])){
  ...
  if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z\s]*$/",$name) && !preg_match("/^[0-9]*$/",$pid)) {
    header("Location: ../signup.php?error=invalidemailunamepid");
    exit();
  }else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: ../signup.php?error=invalidemail&uname=".$name);
    exit();
  }else if (!preg_match("/^[a-zA-Z\s]*$/",$name)) {
    header("Location: ../signup.php?error=invaliduname&email=".$email);
    exit();
  }else if (!preg_match("/^[0-9]*$/",$pid) || strlen($pid) != 10 ) {
    header("Location: ../signup.php?error=invalidpid&email=".$email."&uname=".$name);
    exit();
  }else if ($password !== $passwordRep) {
    header("Location: ../signup.php?error=passwordcheck&email=".$email."&uname=".$name);
    exit();
  }else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultChek> 0) {
        header("Location: ../signup.php?error=emailtaken&uname=".$name);
        exit();
      }else {
